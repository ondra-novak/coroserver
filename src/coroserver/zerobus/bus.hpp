...
public:
    ...
    virtual bool forward_message(IListener *listener, const Message &msg) = 0;
    virtual std::string get_random_channel_name(std::string_view prefix) const = 0;
    virtual bool is_channel(ChannelID id) const = 0;
    virtual ChannelList get_public_channels(const IListener *listener, ChannelListStorage &storage) const = 0;
    virtual ChannelList get_subscribed_channels(const IListener *listener, ChannelListStorage &storage) const = 0;
    virtual ChannelList get_subscribed_groups(const IListener *listener, ChannelListStorage &storage) const = 0;
...
public:
     ...
     *
     * @param listener A pointer to the listener acting as the outgoing bridge. This
     ...
     *
     * @param msg A reference to the message object to be forwarded.
     *
     * @retval true The message was successfully forwarded.
     ...
     */
    bool forward_message(IListener *listener, const Message &msg) {
        return _ptr->forward_message(listener, msg);
    }
     ...
     *         The list will be empty if the listener has no active subscriptions.
     */
    ChannelList get_subscribed_groups(const IListener *listener, ChannelListStorage &storage) const {
     ...
     */
    ChannelList get_public_channels(const IListener *skip_listener, ChannelListStorage &storage) const {
