/*
    ...
    Adapted for userver
        -- Ondrej Novak
        -- removed istream, use C++17, string_view

*/

#ifndef SRC_USERVER_SHA1_H_
#define SRC_USERVER_SHA1_H_
...
public:
     ...
     * @return digest in binary form (each character is byte)
     */
    ...
    ///Finish hashing, return digest
    /**
     * @return digest in binary string
     *
     * @note function also resets internal state
     */
    std::string final()
    {
        ...
        std::string out;
        out.reserve(digest.size()*sizeof(digest[0]));
...
private:
    ...
    static void reset(Digest &digest, std::string &buffer, uint64_t &transforms)
    {
    ...
    static void transform(Digest &digest, uint32_t block[BLOCK_INTS], uint64_t &transforms)
    {
